...
export class LoginService {
    ...
    constructor(private http: HttpClient, private router: Router, private appMetaInfoService: AppMetaInfoService, private serviceMetaInfoService: ServiceMetaInfoService) {
    ...
    public verifyLogin(userid: string, userpsd: string): Observable<any> {
        return Observable.create((observer) => {
            ...
            this.requestLoginService(userid, userpsd, appMetaInfo.appid).subscribe({
                next: configData => {
                    if (configData.status.code !== '200') {
                        observer.next(configData.data.info);
                        console.log(`#login.service# ${configData.status.code} - ${configData.data.info}`);
    ...
    private requestLoginService(account: string, password: string, appid: string, version?: string): Observable<any> {
        let loginPostData: LoginPostData = new LoginPostData(appid, account, password);
